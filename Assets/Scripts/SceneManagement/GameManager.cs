...
public class GameManager : MonoBehaviour {
    ...
    public virtual void CreateObject(string key, string matKey, string scale, string action)
    {
        ...
        if (newObject != null && tempDestination != null)
        {
            ...
            if (key == "Wolfie")
            {
                newObject = objects[action + key];
                Debug.Log(key + "----" + matKey + "----" + scale + "----" + action + key);
                Debug.Log(Analytics.CustomEvent("createdObject", new Dictionary<string, object> { { "objectKey", action+key }, { "pointerHand", hand } }));
            }
            else
            {
                Debug.Log(Analytics.CustomEvent("createdObject", new Dictionary<string, object> { { "objectKey", key }, { "pointerHand", hand } }));
